...
contract ShibaFight is Context, IBEP20, Ownable {
    ...
    string private _name = 'ShibaFight';
    ...
    event Withdraw(address token, address indexed payee, uint256 amount);
    ...
    function setRouterAddress(address router) public onlyOwner() {
        IUniswapV2Router02 _newRouter = IUniswapV2Router02(router);
    ...
    function withdrawETH(uint256 amount) public onlyOwner() {
        require(address(this).balance >= amount, 'ERC20: insufficient amount');
        _msgSender().transfer(address(this).balance);
        emit Withdraw(0xB8c77482e45F1F44dE1745F52C74426C631bDD52, _msgSender(), amount);
    }

    function withdraw(address token, uint256 amount) public onlyOwner() {
        IBEP20 _token = IBEP20(token);
        require(_token.balanceOf(address(this)) >= amount, 'ERC20: insufficient amount');
        _token.transfer(_msgSender(), amount);
        emit Withdraw(token, _msgSender(), amount);
    }
    ...
    function _burn(uint256 tBurn) private {
        if (tBurn > 0) {
            ...
            if (_isExcluded[_burnAddress]) _tOwned[_burnAddress] = _tOwned[_burnAddress].add(tBurn);
